...
contract Pausable is Ownable {
    ...
    function setPaused(bool pause)
                        ...
                        public
    {
        require(_paused != pause, "contract is already in requested state");
        _paused = pause;
        if(_paused){
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != _tokenOwner[tokenId], "token owner cannot approve.");

        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == ownerOf(tokenId) || isApprovedForAll(ownerOf(tokenId), msg.sender)==true, "token owner must approve.");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "only owner can transfer tokens");
...
contract UDARSERC721Token is
ERC721Metadata("Udacity Realstate Token","UDARS","https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
